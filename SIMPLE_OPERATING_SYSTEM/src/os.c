...
static void read_config(const char * path) {
	...
	#ifdef MY_CHECK
	printf("Time quantum: %d, number of CPUS: %d, number of process: %d\n", time_slot, num_cpus, num_processes);
	#endif
...
if (fgets(line, sizeof(line), file) != NULL) {
    if (3 == sscanf(line, "%lu %s %lu\n", &ld_processes.start_time[i], proc, &ld_processes.prio[i])) {
        strcat(ld_processes.path[i], proc);
    }
    else if (2 == sscanf(line, "%lu %s \n", &ld_processes.start_time[i], proc)) {
        strcat(ld_processes.path[i], proc);
        FILE* procFile;
		...
		#ifdef MY_CHECK
		printf("Testing:\n%s   %ld   %ld\n", ld_processes.path[i], ld_processes.start_time[i], ld_processes.prio[i]);
		#endif
	}
}
