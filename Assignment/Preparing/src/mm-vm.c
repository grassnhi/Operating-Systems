...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  { // if successfully get free region
  ...
  if (size <= cur_vma->vm_end - cur_vma->sbrk){  // There is enough gap between sbrk and vm_end
   ...
   */
  int new_inc = old_sbrk + size - cur_vma->vm_end;
  int inc_sz = PAGING_PAGE_ALIGNSZ(new_inc);
...
int __free(struct pcb_t *caller, int vmaid, int rgid)
{
  ...
  int num = PAGING_PAGE_ALIGNSZ(rgnode->rg_end - rgnode->rg_start) / PAGING_PAGESZ;
  int fpn = PAGING_FPN(rgnode->rg_start);
  int pgn = PAGING_PGN(rgnode->rg_start);

  for(int pgit = 0; pgit < num; pgit++){
    uint32_t pte = caller->mm->pgd[pgn + pgit];
    if(PAGING_PAGE_PRESENT(pte)){
      MEMPHY_put_freefp(caller->mram, fpn + pgit);
    } 
  }
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    uint32_t vicpte = caller->mm->pgd[vicpgn]; // get PTE for victpgn
		int vicfpn = PAGING_FPN(vicpte); // extrac fpn
...
int __write(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE value)
{
  ...
  if(currg->rg_start + offset < cur_vma->vm_start || currg->rg_start + offset >= cur_vma->sbrk){ /* Invalid memory access */
    return -1;
  }

  if (pg_setval(caller->mm, currg->rg_start + offset, value, caller) == -1){
    return -1;
  }
...
int inc_vma_limit(struct pcb_t *caller, int vmaid, int inc_sz)
{
  ...
  struct vm_rg_struct *area = get_vm_area_node_at_brk(caller, vmaid, inc_sz, inc_amt); // newly allocated region from [sbrk , sbrk + inc_sz]
  struct vm_area_struct *cur_vma = get_vma_by_num(caller->mm, vmaid);

  int old_end = cur_vma->vm_end;

  /*Validate overlap of obtained region */
  if (validate_overlap_vm_area(caller, vmaid, area->rg_start, area->rg_end) < 0) // if the new region [sbrk , sbrk + inc_sz] overlaps with any existing memory regions, 
    return -1; /*Overlap and failed allocation */ // return -1 if not (meaning sbrk is more than end)
...
void update_LRU(struct mm_struct *mm, int pgn) {
  struct LRU_node *pgit = mm->LRU_pgn;
  while (pgit)
  {
    if (pgit->pgn == pgn)
    {
      struct LRU_node *topPgn = pgit;
      pgit = pgit->pgn_next;
      topPgn->pgn_next = mm->LRU_pgn;
      mm->LRU_pgn = topPgn;
    }
    else
      pgit = pgit->pgn_next;
  ...
  // If pgn doesn't exist, create a new node
  struct LRU_node *newPgn = (struct LRU_node *)malloc(sizeof(struct LRU_node));
  newPgn->pgn = pgn;
  newPgn->pgn_next = mm->LRU_pgn;
  mm->LRU_pgn = newPgn;
}

struct LRU_node *LRU_last_node(struct mm_struct *mm) {
  struct LRU_node *current = mm->LRU_pgn;
  struct LRU_node *prev = NULL;

  while (current && current->pgn_next) {
    prev = current;
    current = current->pgn_next;
  }

  if (prev != NULL) {
    prev->pgn_next = NULL;
  } else {
    mm->LRU_pgn = NULL; // If there's only one node
  }
